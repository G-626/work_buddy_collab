...
def teacher_guide(name: str, tr: dict, date_s: str, brief: str, script: str,
                  student: str) -> str:
    counts = "; ".join(f"{k}: {v}" for k, v in tr["counts"].items())
    if student == "marco":
        snapshot = (f"{name} communicates best with clear, literal language and one "
                    "instruction at a time.\nNew or loud situations can be hard; advance "
                    "warning and written steps help a lot.")
        expect = ("- May pause before the next step and not ask — waiting briefly, then "
                  "using the script below, works.\n"
                  "- In a loud environment, may reach for ear support or cover ears. "
                  "This is self-regulation, not refusal.\n"
                  "- Asks precise questions when unsure; vague answers (\"maybe\", "
                  "\"we'll see\") cause confusion.")
        helps = ("- One instruction at a time, in literal language — no idioms.\n"
                 "- If there is a pause before the next step, wait a few seconds, then "
                 "use the script.\n"
                 "- Give a written step list when the task has more than two steps.\n"
                 "- Offer the ear support before the machine starts, so it is a choice, "
                 "not a rescue.")
        avoid = ("- Abstract or sarcastic phrasing (\"hold your horses\").\n"
                 "- Repeating a question before waiting ~5 seconds.\n"
                 "- Raising your voice over the machine — the ear support is the tool, "
                 "not volume.")
        warning = ("- Pauses longer than ~30 seconds before the next step.\n"
                   "- Hands moving toward ears without the ear support nearby.\n"
                   "- Looks at the exit repeatedly during a new activity.")
    else:  # priya
        snapshot = (f"{name} communicates best in writing and in structured settings. "
                    "She is highly articulate in class discussions\nbut goes quiet in "
                    "unstructured social time. She masks heavily — copying peers to blend "
                    "in — then exhausts.")
        expect = ("- May go quiet and stop participating when the class is asked to "
                  "\"work together\" without assigned roles.\n"
                  "- Will not ask for help in front of peers — she perceives it as "
                  "exposure. She asks questions by email or after class instead.\n"
                  "- A neutral correction (\"try again\") can be heard as failure — "
                  "she needs feedback framed as process, not judgement.")
        helps = ("- Assign a specific role in group work — \"You are the note-taker\" "
                 "works; \"work together\" does not.\n"
                 "- Give feedback privately — written comments on work, or a quiet word "
                 "after class, never in front of peers.\n"
                 "- Warn about changes in advance — even small ones (room change, supply "
                 "teacher). A note in her planner the day before prevents escalation.\n"
                 "- Allow sketching during listening tasks — it regulates her and does "
                 "not mean she isn't paying attention.")
        avoid = ("- Public praise or correction in front of the class — this causes "
                 "visible distress, then withdrawal.\n"
                 "- Ambiguous instructions like \"be creative\" or \"work together\" — "
                 "she freezes and produces nothing.\n"
                 "- Assuming she is fine because she seems fine — she masks heavily and "
                 "exhausts at home.")
        warning = ("- Going quiet and dropping eye contact during group activities.\n"
                   "- Erasing or restarting written work repeatedly.\n"
...
def social_story(name: str, date_s: str, student: str) -> str:
    if student == "marco":
        story = (
    ...
    else:  # priya — third person per passport voice preference
        story = (
...
def therapist_summary(name: str, tr: dict, date_s: str, notes: dict,
                      student: str) -> str:
    progress_rows = "".join(
        f"| {g} | {ev} | {st} |\n" for g, ev, st in notes["progress"])
    counts = "; ".join(f"{k}: {v}" for k, v in tr["counts"].items())
    if student == "marco":
        passport_summary = (
            "Communication: literal, precise; does not ask for help spontaneously — "
            "needs explicit permission scripts. Triggers: loud machine noise, transitions "
            "to unfamiliar places. What works: advance preview, written steps, ear "
            "support, scripts.")
        open_questions = (
            "- Confirm whether the ~45 s and ~30 s pauses correlate with step-transition "
            "timing across sessions, or with something else in the environment.\n"
            "- Validate the self-initiated regulation event (10:15) against the "
            "transcript line.")
    else:  # priya
        passport_summary = (
            "Communication: highly articulate in structured settings, goes quiet in "
            "unstructured social time; masks heavily. Triggers: being singled out, "
            "unstructured social time, ambiguous instructions. What works: private "
            "feedback, assigned roles, advance warning, art as regulation.")
        open_questions = (
            "- The 2-second look-down before answering at 11:15 — is this consistent "
            "with her private feedback preference, or a pattern to watch in future "
            "group sessions?\n"
            "- 11:18: Ms. Liu's acknowledgement was public. Priya showed no visible "
...
"""
    ...
    # --- Step 5: review gate ----------------------------------------------
    if student == "marco":
        gate_edit = ("- Q1 edit: \"Pauses occurred between task steps during the session; record in\n"
                     "  patterns and reassess after ≥3 sessions.\"\n"
                     "- Q2: confirmed self-initiated — documented as a positive micro-event.")
    else:  # priya
        gate_edit = ("- Q1 edit: \"2-second look-down is within normal range for Priya in group\n"
                     "  settings; record in patterns and compare across sessions.\"\n"
                     "- Q2: noted — no action needed.")
...
> Nothing merges without this sign-off (ADR-0004 review gate)."""
    ...
    views = {
        ...
        "social-story.md": social_story(name, date_s, student),
        "therapist-summary.md": therapist_summary(name, tr, date_s, notes, student),
    }
...
> **Level-1 dry-run engine.** This report shows the {len(checks)} checks implemented by
